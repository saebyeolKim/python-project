...
class GetNoteReponse(BaseModel):
    total_count: int
    page: int
    notes: list[NoteResponse]

@router.get("", response_model=GetNoteReponse)
@inject
def get_notes(
    page: int = 1,
    ...
    res_notes = []
    for note in notes:
        note_dic = asdict(note)
        note_dic.update({"tags": [tag.name for tag in note.tags]})
        res_notes.append(note_dic)
...
@router.get("/tags/{tag_name}/notes", response_model=GetNoteReponse)
@inject
